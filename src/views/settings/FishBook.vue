<template>
    <div class="settings-fish-book">
        <el-form size="small" :model="form" label-position="left" label-width="120px">
            <el-alert class="tip" title="打开小说阅读器后，再调整下面的设置项，可直接预览效果哦！" type="info"></el-alert>
            <el-form-item label="每页字数">
                <el-slider v-model="form.wordsPerPage" :min="5" :max="200" :step="1" :marks="marksWordsTotal"
                    @change="changeConfig('wordsPerPage')" style="width: 420px"></el-slider>
            </el-form-item>
            <br />
            <el-form-item label="字体颜色">
                <el-color-picker v-model="form.textColor" @change="changeConfig('textColor')" show-alpha
                    style="width: 100px"></el-color-picker>
            </el-form-item>
            <el-form-item label="背景颜色">
                <el-color-picker v-model="form.textBgColor" @change="changeConfig('textBgColor')" color-format="hex"
                    show-alpha style="width: 100px"></el-color-picker>
                <span class="color-picker-tip">
                    Tip：支持调整背景色透明度，调整为 0，则只显示文字
                </span>
            </el-form-item>
            <el-form-item label="窗口透明度">
                <el-slider v-model="form.bgOpacity" :min="0.1" :max="1" :step="0.01" :marks="marksBgOpacity"
                    @change="changeConfig('bgOpacity')" style="width: 420px"></el-slider>
            </el-form-item>
            <br />
            <el-form-item label="字体大小">
                <el-slider v-model="form.fontSize" :min="12" :max="30" :step="1" @change="changeConfig('fontSize')"
                    style="width: 420px"></el-slider>
            </el-form-item>
            <br />
            <el-form-item label="字体间距">
                <el-slider v-model="form.letterSpacing" :min="0" :max="20" :step="0.5"
                    @change="changeConfig('letterSpacing')" style="width: 420px"></el-slider>
            </el-form-item>
            <el-form-item label="阅读器窗口">
                <el-checkbox v-model="form.resizable" true-label="1" false-label="0" @change="changeConfig('resizable')">
                    允许调整宽高
                </el-checkbox>
            </el-form-item>
            <br />
